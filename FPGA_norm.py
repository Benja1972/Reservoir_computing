...
params = {
    ...
    'Phi0'  : 0.25*np.pi,
    #~ 'theta' : 1.59e-3/7.958e-6,  #theta(s)/tau(s)  # 0.8 ms < theta< 1*1000 ms
   'theta' : 79.5e-3/7.958e-6,  #theta(s)/tau(s)  # 0.8 ms < theta< 1*1000 ms
...
tfinal=35000
tcut=30000
dde.set_sim_params(tfinal=tfinal, dtmax=0.5, AbsTol=10**-4, RelTol=10**-2)
...
#~ pl.figure(11)
#~ pl.plot(t*tau_real,x1)
#~ pl.axis('tight')
#~ pl.xlabel('$Time$')
#~ pl.ylabel('$x(t)$')
#~ pl.title(r'$\beta=$ %1.1f' 
            #~ % (beta ) )

f, (ax1, ax2) = pl.subplots(2, 1, sharex=True)

#pl.subplot(211)
ax1.plot(t*tau_real,x1)
#~ pl.xlabel('$t$')
ax1.set_ylabel('$x(t)$')
ax1.set_title(r'$\beta=$ %1.3f,  $\theta=$ %1.2f, $\tau=$ %1.2f, T= %1.2f' 
            % (beta, theta, tau, T ) )

#~ pl.subplot(212)
ax2.plot(t*tau_real,y1)
ax2.set_xlabel('$t$')
ax2.set_ylabel('$y(t)$')
pl.axis('tight')
